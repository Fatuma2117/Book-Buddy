...
router.get("/", rejectUnauthenticated, (req, res) => {
  ...
  ;
  `;
  const sqlValues = [req.headers.currentkidid, true];
  pool
    .query(sqlQuery, sqlValues)
    .then((dbRes) => {
      res.send(dbRes.rows);
...
router.put("/:id", (req, res) => {
  ...
  `;
  const sqlValues = [true, req.params.id, req.headers.currentkidid];
...
router.delete("/:id", (req, res) => {
  ...
  const sqlValues = [req.headers.currentkidid, req.params.id];
